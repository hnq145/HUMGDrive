...
export const getFoldersListAPI = async ({
  ...
  const [, { parent, search, sortBy, limit, trashMode }] = queryKey;
...
>) => {
  const [, { parent, search, folderIDs, currentParent }] = queryKey;
...
export const uploadFolderAPI = async (data: FormData, config: AxiosRequestConfig) => {
