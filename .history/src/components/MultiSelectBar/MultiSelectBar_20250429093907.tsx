...
const MultiSelectBar: React.FC = () => {
  ...
  const trashItems = async () => {
    try {
      ...
      const itemsToTrash = Object.values(multiSelectMap).map((item) => item.id);
  ...
  const deleteItems = async () => {
    try {
      ...
      const itesmsToDelete = Object.values(multiSelectMap).map(
        (item) => item.id
      );
  ...
  const restoreItems = async () => {
    ...
    const itemsToRestore = Object.values(multiSelectMap).map((item) => item.id);
